...
# setup files
audit_plist_managed="/Library/Managed Preferences/org.800-53r5_low.audit.plist"

if [[ ! -e "$audit_plist_managed" ]];then
    audit_plist_managed="/Library/Preferences/org.800-53r5_low.audit.plist"
fi

audit_plist="/Library/Preferences/org.800-53r5_low.audit.plist"
audit_log="/Library/Logs/800-53r5_low_baseline.log"

lastComplianceScan=$(defaults read /Library/Preferences/org.800-53r5_low.audit.plist lastComplianceCheck)
...
compliance_count(){
    ...
    results=$(/usr/libexec/PlistBuddy -c "Print" /Library/Preferences/org.800-53r5_low.audit.plist)
...
if [[ $(/usr/bin/tail -n 1 "$audit_log" 2>/dev/null) = *"Remediation complete" ]]; then
 	echo "$(date -u) Beginning 800-53r5_low baseline scan" >> "$audit_log"
else
 	echo "$(date -u) Beginning 800-53r5_low baseline scan" > "$audit_log"
fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "2" ]]; then
            echo "$(date -u) auth_pam_login_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_pam_login_smartcard_enforce -dict-add finding -bool NO
        else
            echo "$(date -u) auth_pam_login_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "2" ]]; then
            echo "$(date -u) auth_pam_su_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_pam_su_smartcard_enforce -dict-add finding -bool NO
        else
            echo "$(date -u) auth_pam_su_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "2" ]]; then
            echo "$(date -u) auth_pam_sudo_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_pam_sudo_smartcard_enforce -dict-add finding -bool NO
        else
            echo "$(date -u) auth_pam_sudo_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "1" ]]; then
            echo "$(date -u) auth_smartcard_allow passed (Result: $result_value, Expected: "{'integer': 1}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_smartcard_allow -dict-add finding -bool NO
        else
            echo "$(date -u) auth_smartcard_allow failed (Result: $result_value, Expected: "{'integer': 1}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "1" ]]; then
            echo "$(date -u) auth_smartcard_enforce passed (Result: $result_value, Expected: "{'integer': 1}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_smartcard_enforce -dict-add finding -bool NO
        else
            echo "$(date -u) auth_smartcard_enforce failed (Result: $result_value, Expected: "{'integer': 1}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "2" ]]; then
            echo "$(date -u) auth_ssh_password_authentication_disable passed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
            defaults write "$audit_plist" auth_ssh_password_authentication_disable -dict-add finding -bool NO
        else
            echo "$(date -u) auth_ssh_password_authentication_disable failed (Result: $result_value, Expected: "{'integer': 2}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "7d" ]]; then
            echo "$(date -u) audit_retention_configure passed (Result: $result_value, Expected: "{'string': '7d'}")" | tee -a "$audit_log"
            defaults write "$audit_plist" audit_retention_configure -dict-add finding -bool NO
        else
            echo "$(date -u) audit_retention_configure failed (Result: $result_value, Expected: "{'string': '7d'}")" | tee -a "$audit_log"
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    if [[ ! $exempt == "true" ]] || [[ -z $exempt ]];then
        if [[ $result_value == "60" ]]; then
            echo "$(date -u) pwpolicy_60_day_enforce passed (Result: $result_value, Expected: "{'integer': 60}")" | tee -a "$audit_log"
            defaults write "$audit_plist" pwpolicy_60_day_enforce -dict-add finding -bool NO
        else
            echo "$(date -u) pwpolicy_60_day_enforce failed (Result: $result_value, Expected: "{'integer': 60}")" | tee -a "$audit_log"
